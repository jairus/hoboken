...
function ecpt_register_taxonomies() {
	...
	foreach( $taxonomies as $key => $tax) {
		...
		$labels = array(
			...
			'add_new' 						=> _x( 'Add New ' . utf8_decode($tax->singular_name), 'taxonomy add new'),
			...
			'popular_items' 				=> __( 'Popular ' . utf8_decode($tax->plural_name) ),
			'all_items' 					=> __( 'All ' . utf8_decode($tax->plural_name) ),
			'view_item' 					=> __( 'View ' . utf8_decode($tax->singular_name) ),
			'separate_items_with_commas'	=> __( 'Separate ' . utf8_decode($tax->plural_name) . ' with commas' ),
			'choose_from_most_used'			=> __( 'Choose from the most used ' . utf8_decode($tax->plural_name)),
		...
		$args = array(
			...
			'rewrite' 			=> array('slug' => utf8_decode($tax->slug), 'with_front' => $with_front),
