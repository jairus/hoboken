...
function ecpt_options_upgrade() {
	...
	****************************************/
	...
	if(!$wpdb->query("SELECT `labels` FROM `" . $ecpt_db_name . "`")) 
	{
		$wpdb->query("ALTER TABLE `" . $ecpt_db_name . "` ADD `labels` mediumtext");
		update_option('ecpt_db_version', $ecpt_db_version );	
	}
	// check to see if the advanced_supports column needs added for post types
	if(!$wpdb->query("SELECT `advanced_supports` FROM `" . $ecpt_db_name . "`")) 
	{
		$wpdb->query("ALTER TABLE `" . $ecpt_db_name . "` ADD `advanced_supports` mediumtext");
		update_option('ecpt_db_version', $ecpt_db_version );	
	}
	...
	****************************************/
	...
	{
		$wpdb->query("ALTER TABLE " . $ecpt_db_meta_name . " ADD `post_ids` mediumtext");
...
function ecpt_upgrade_table_collation() {
	if(isset($_GET['ecpt-action']) && $_GET['ecpt-action'] == 'db-collate' && !get_option('ecpt_dbs_collated')) {
		...
		$wpdb->query("alter table `" . $ecpt_db_name . "` convert to character set utf8 collate utf8_unicode_ci");
		$wpdb->query("alter table `" . $ecpt_db_tax_name . "` convert to character set utf8 collate utf8_unicode_ci");
		$wpdb->query("alter table `" . $ecpt_db_meta_name . "` convert to character set utf8 collate utf8_unicode_ci");
		$wpdb->query("alter table `" . $ecpt_db_meta_fields_name . "` convert to character set utf8 collate utf8_unicode_ci");
